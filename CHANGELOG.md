...
- Notification `Needs you` quand un run s'arrête sur une carte (`run.halted`), retirée dès que l'opérateur la fait bouger (f69d254)
- Notification `Done` quand tous les membres d'un run sont déposés (`run.finished`), retirée dès qu'un membre bouge (f69d254)
...
- Les tests de `collie-board-ctl` n’héritent plus de la config du service : un push lancé par le bridge n’échoue plus au hook `pre-push` (50d4974)
...
- Revue du copilote : plus aucune carte sur diff vide ou verdict `complete`, le nettoyage de branche devient une action, les doublons re-titrés sont refusés et journalés (`copilot.review_filtered`) (f94310d)
...
- Lecteur d'artefacts : depuis l'Historique d'un pane, liste des images/HTML/Markdown écrits ou mentionnés par la session (racine = worktree de la carte, confinement realpath) — rendus en image, Markdown, ou HTML frotté et sandboxé (350a204)

## [0.151.2] - 2026-09-22

### Fixed

- Preview d'images absente en lecture sur les panes OpenCode : les images retournées inline par un outil (`{type:"file", uri:"data:image/…"}`) sont désormais rendues comme pour les transcripts Claude, avec leur taille bornée au même plafond. (a8b1b58)
...
- Choix du harness par carte (Claude / Codex / Cursor / OpenCode) : à la création, dans l'éditeur et avant Start. (0129029)
...
- En desktop, les repos et tags du panneau « Filter » passent à la ligne au lieu d'être rognés par le bord (caf6f80)

## [0.142.0] - 2026-09-08

### Added

- Un bouton « Relancer la review » sur la carte : le copilote rejuge sur le diff actuel de la branche, donc un `partial` corrigé depuis n'affiche plus le verdict du premier passage (396e9b0)
...
- La review est refusée quand la carte n'a plus de worktree, au lieu de rendre un verdict sur `(no worktree for this card)`
- Une review relancée ne refile plus les follow-ups déjà créés par la précédente
- `listReviews` départage deux reviews de la même milliseconde, pour que la plus récente reste en tête
