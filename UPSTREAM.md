...
| `bridge/notify-prefs.ts` · `web/src/lib/types.ts` · `web/src/components/notify-prefs-control.tsx` | a fifth switch, `board` (default on) — one boolean for the whole family of the board's own alerts, never one per event — card N6, fork-only. NOT in the ledger: there is no board upstream to have alerts about |
