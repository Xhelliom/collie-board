...
| `bridge/index.ts` | construct the board, copilot and adapters; **seven** `engine.onUpdate` hooks (reconcile · context · handoffs · auto handoff · wrapups · copilot review · the board's journal tailer); the `NotifyLog` and its coordinator hook — brick 18 in the ledger; the two notification hooks skip a paneless alert — card N6, fork-only; the transition hook skips the spell the board's own automatic handoff prompt causes (a question still goes through) — fork-only |
