...
| File | Change |
|---|---|
| `bridge/server.ts` | `parseNotifyPrefsPatch` takes its key list from `DEFAULT_NOTIFY_PREFS` instead of carrying its own copy — a preference added elsewhere was silently unwritable; `historyParams` takes the `after` cursor; one import + one dispatch block for the board's paths (`/api/cards`, `/api/repos`, `/api/board`, `/api/backup`, `/api/runs`), plus `board`/`copilot`/`context` in the options; `withCardFields()` + `ContextTracker.enrich()` overlaid onto the primary session's snapshot panes; the `GET /api/notifications/log` + `DELETE /api/notifications/log/:id` + `POST /api/notifications/log/:id/read` routes and the snapshot's `notifications.count` — brick 18 in the ledger; one import + one dispatch block for `/api/gallery` — brick 21 |
| `bridge/index.ts` | construct the board, copilot and adapters; **eight** `engine.onUpdate` hooks (reconcile · context · handoffs · auto handoff · wrapups · copilot review · the board's journal tailer · the run coordinator — `runHook` from `run.ts`, ADR 0017); the transition hook silences the run's lead pane (`isLeadPane`) as it does the copilot's — fork-only; the `NotifyLog` and its coordinator hook — brick 18 in the ledger; the two notification hooks skip a paneless alert — card N6, fork-only; the transition hook skips the spell the board's own automatic handoff prompt causes (a question still goes through) — fork-only |
| `bridge/notifications.ts` | one optional constructor argument (`onFire`) and the call that feeds the bell's history — brick 18 in the ledger; `summarize`'s single-alert branch composes through `notify-content.ts` — brick 24; **the deepest cut of the four**: a second optional argument (`subtitleFor`) that the fired alert is now AWAITED on, an `Alert.subtitle` field `summarize` renders, and the timer callback split out into an `async fire()` with a resolved-while-we-waited guard. Deliberate and priced in ([NOTIFY_AUDIT.md §N10](./NOTIFY_AUDIT.md)): a push's body arriving in a second, silent message never reaches a sleeping phone, because every message shares one collapse topic — verified on a locked device. Only the copilot is slow enough to earn a second message; the free tiers cost ~80ms against a 30s debounce, so the first push waits for them. It makes brick 24's `notify-content.ts` extraction heavier — that is the bill; `summarize`'s digest body lists the SUBJECTS instead of `paneDisplayName` — brick 25; that second argument then widened from `subtitleFor` to `beforeFire`, returning `{ subtitle, cardStatus }` — the card's status can only be read once the debounce is up (§4.2), and it carries `Alert.cardStatus` + `HerdSummary.cardId` so a finished card's alert says `Review` and taps through to the card — card N4, fork-only; **the second deep cut**: the coordinator no longer assumes an alert is a pane — `arm(key, alert)`/`retract(key)` public over an opaque key, `paneId` moved onto `Alert` as optional (so `FiredAlert` is now an alias), the deep-link read off the alert instead of the map key, `retract()` public in place of a private `resolve()` (and of `onTransition`'s second copy of the same rule), and a `stalled` status with its marker and digest line. Deliberate and priced in ([NOTIFY_AUDIT.md §6.4](./NOTIFY_AUDIT.md)) — brick 28 in the ledger, and the only part of it that is fork-shaped is the word `stalled`; a second such word, `ready`, with its marker and its digest line in last place — a card a finished predecessor has just unblocked — card N6/B4, fork-only |
| `bridge/config.ts` | the `board*` config block |
| `bridge/push.ts` · `web/src/lib/push-decision.ts` · `web/src/sw.ts` | one optional `cardId` alongside the existing `target`, carried into the payload's `data` and read by `notificationclick`: an alert about a card to READ opens that card instead of the pane its agent finished in ([NOTIFY_AUDIT.md §4.1](./NOTIFY_AUDIT.md)) — fork-only, and NOT in the ledger: there is no card upstream to open. Additive on all three: an older cached SW ignores the field and takes the pane path |
...
| `bridge/transcript.ts` | `latestUsage()`, `resolveByCwd()`, `resolveWithoutSession()`, `pageAt()`, and `pageEntries`' `after` cursor; `toolImagePath()` + `toolResultImage()` + the tool part's `image` field — brick 21 in the ledger |
...
| `bridge/types.ts` | `AgentView.branch` (card-backed panes only) and `ctxPct/ctxTokens` (any agent pane — G1/G2/G3); `notifications.count` — brick 18 in the ledger ; `AgentView.statusSince` — brick 22; `AgentView.cardStatus`, beside `branch` and from the same lookup — card N4, fork-only; `AgentView.handoffReady`, same gating — fork-only |
...
| `bridge/notify-prefs.ts` · `web/src/lib/types.ts` · `web/src/components/notify-prefs-control.tsx` | a fifth switch, `board` (default on) — one boolean for the whole family of the board's own alerts, never one per event — card N6, fork-only; then a sixth, `ready` (default **off**), which is a second family and not a sixth event: it opens a possibility where `board` reports a problem — card N6/B4, fork-only. NOT in the ledger: there is no board upstream to have alerts about |
...
Rebasing means resolving those rows, not the whole tree. Keep it that way: if a change wants to
