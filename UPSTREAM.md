...
| `bridge/index.ts` | construct the board, copilot and adapters; **six** `engine.onUpdate` hooks (reconcile · context · handoffs · wrapups · copilot review · the board's journal tailer); the `NotifyLog` and its coordinator hook — brick 18 in the ledger; the two notification hooks skip a paneless alert — card N6, fork-only |
...
| `bridge/types.ts` | `AgentView.branch` (card-backed panes only) and `ctxPct/ctxTokens` (any agent pane — G1/G2/G3); `notifications.count` — brick 18 in the ledger ; `AgentView.statusSince` — brick 22; `AgentView.cardStatus`, beside `branch` and from the same lookup — card N4, fork-only |
