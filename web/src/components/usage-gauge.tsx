...
// How much Claude Code quota is LEFT, on the dashboard. The bridge reads it from `claude -p /usage`
...
/** What the panel's percentages mean here: it prints what's USED, the gauge shows what's left. */
function left(percent: number): number {
      ...
      {usage.limits.map((limit) => (
        <div key={limit.label} className="flex flex-col gap-1">
          <div className="flex items-baseline gap-2 text-xs text-muted-foreground">
            ...
            <span className="tabular-nums text-foreground">{left(limit.percent)}%</span>
            {limit.resetsAt && (
              <span className="ml-auto truncate text-[11px]">resets {limit.resetsAt}</span>
            )}
          </div>
          <div className="h-1.5 overflow-hidden rounded-full bg-muted">
            {/* The bar fills with what's LEFT, tinted by what's USED — so it empties and reddens as
                the quota goes, which is the direction the eye expects from a fuel gauge. */}
            <div
              className={cn("h-full rounded-full transition-all", ctxTone(limit.percent))}
              style={{ width: `${left(limit.percent)}%` }}
            />
          </div>
        </div>
      ))}
