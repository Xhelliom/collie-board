...
// see it: <App> (which renders the cover) and use-polling's tick (which must NOT fetch behind it). A
// store rather than context because the polling tick reads it from inside a `setInterval` callback,
// where a captured render value would go stale — `isLocked()` is a live read at fire time and costs
// no re-render. Same `useSyncExternalStore` shape as lib/status.ts.
//
// It also carries the catch-up beat. Resuming has to refetch (the route tree stays mounted, so no
