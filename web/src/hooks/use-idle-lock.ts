...
//   • Returning to the foreground AUTO-RESUMES. A lock met on the way back is pure ceremony: it
...
// past the deadline (a desk or a tablet; almost never a phone, whose screen sleeps long first). That
// is the case where covering the screen and dropping the poll actually earns its keep.
//
...
export function useIdleLock(idleMs = DEFAULT_IDLE_MS): { locked: boolean; unlock: () => void } {
  ...
  useEffect(() => {
    ...
    function check() {
      ...
      // re-arming on an already-elapsed deadline would spin a 0ms timeout. onVisibility restarts it.
