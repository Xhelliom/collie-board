...
describe("coerceNotifyPrefs", () => {
  test("fills missing / non-boolean keys from defaults", () => {
    expect(coerceNotifyPrefs(undefined)).toEqual(DEFAULT_NOTIFY_PREFS);
    expect(coerceNotifyPrefs(null)).toEqual(DEFAULT_NOTIFY_PREFS);
    expect(coerceNotifyPrefs({})).toEqual(DEFAULT_NOTIFY_PREFS);
    expect(coerceNotifyPrefs({ blocked: false })).toEqual({ ...DEFAULT_NOTIFY_PREFS, blocked: false });
    expect(coerceNotifyPrefs({ done: true })).toEqual({ ...DEFAULT_NOTIFY_PREFS, done: true });
    // `updates` is a first-class key: an explicit false sticks, non-booleans fall back to the default.
    expect(coerceNotifyPrefs({ updates: false })).toEqual({ ...DEFAULT_NOTIFY_PREFS, updates: false });
    // Same for `copilotSubtitle` — off by default, sticks when explicitly set.
    expect(coerceNotifyPrefs({ copilotSubtitle: true })).toEqual({
      ...DEFAULT_NOTIFY_PREFS,
      copilotSubtitle: true,
    });
    expect(
      coerceNotifyPrefs({ blocked: "yes", done: 1, updates: 0, copilotSubtitle: "no" }),
